    ...
    <div class="col-12">
        ...
        <nav class="breadcrumb-container d-none d-sm-block d-lg-inline-block" aria-label="breadcrumb">
            <ol class="breadcrumb pt-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">{{ $store->name }} Products</li>
            </ol>
        </nav>
    ...
    <div class="col-12">
        <div class="card">
                <div class="card-body">
                    <div class="row">
                      <div class="col-5">
                        {{ Form::open(['url'=>'admin/products/'.$id,'method'=>'GET']) }}
                      ...
                      <div class="col-7 text-right">
                          <a href="{{ URL::to('admin/products/import/'.$id) }}" class="btn btn-md btn-primary add_btn">Import Product</a>
                          <a  class="btn btn-md btn-primary add_btn" data-toggle="modal" data-target="#exampleModal_offer">Add Offer </a>
                          <a  class="btn btn-md btn-primary add_btn" href="{{ URL::to('admin/store/offers/'.$id) }}">All Offer </a>
                          @if($store->department_id == 2)
                                 <a href="{{ URL::to('admin/restaurant/items/add/'.$id) }}" class="btn btn-md btn-primary add_btn">Add New Product</a>
                          @else
                                 <a href="{{ URL::to('admin/products/add/'.$id) }}" class="btn btn-md btn-primary add_btn">Add New Product</a>
                          @endif
                          <a href="{{ URL::to('admin/restaurant/items/add/'.$id) }}" class="btn btn-md btn-primary add_btn">Add New Item</a>
                           <a href="{{ URL::to('admin/restaurant/menu/'.$id) }}" class="btn btn-md btn-primary add_btn">Menu Items</a>
                      </div>
                    </div>
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <th scope="row"> {{ $data->order_gno }}</th>
                                <td>{{ $data->name }}</td>
                                <td>{{ $data->partner_sku }}</td>
                                <td>{{ ($data->menu)?$data->menu->name:' ' }}</td>
                                <td>
                                    <a href="{{ URL::to('admin/produt/offers/'.$data->id) }}" class="add_icon">
                                    ...
                                    <div class="modal fade" id="exampleModal_offer_{{ $data->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                      <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                          ...
                                          {{ Form::open(['url'=>'admin/product/offer/'.$data->id,'class'=>'ajax_form ']) }}
                                ...
                                <td>
                                    <a href="{{ URL::to('admin/restaurant/items/edit/'.$data->id) }}" class="btn btn-md btn-primary">Edit</a>
...
<div class="modal fade" id="exampleModal_offer" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      {{ Form::open(['url'=>'admin/add/offer/'.$id,'class'=>'ajax_form']) }}
      <h2 class="form_error"></h2>
      <div class="modal-body">
        ...
        <div class="input-group mb-3">
            ...
            <div class="custom-file">
               {{ Form::select('menu_id',$menu,old('menu_id'),['class'=>'form-control js-example-basic-single','min'=>0,'placeholder'=>'Choose Category']) }}
            </div>
        </div>
        @endif
        <div class="input-group mb-3">
